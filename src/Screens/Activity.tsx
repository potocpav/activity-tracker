...
const ActivityInner: React.FC<{ activity: ActivityType; activityPath: ActivityPath; navigation: any }> = ({
  activity,
  activityPath,
  navigation,
}) => {
    ...
    const dataRows = activity.dataPoints.map((dp: DataPoint) => {
      const values = (() => {
        switch (activity.unit.type) {
          ...
          case "multiple":
            return activity.unit.values.map((u) =>
        ...
        >
          <Menu.Item
            onPress={() => {
              ...
              navigation.navigate("EditActivity", { activityPath });
            }}
            leadingIcon="pencil"
            title="Edit"
          />
...
export default withActivityLock(Activity);
